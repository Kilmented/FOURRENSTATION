...
/obj/item/card/id/Destroy()
	...
	// if (my_store) // Money Rework Removal
	// 	QDEL_NULL(my_store)
	return ..()
...
/obj/item/card/id/afterattack_secondary(atom/target, mob/user, proximity_flag, click_parameters)
	...
	if(!proximity_flag || !check_allowed_items(target) || !isfloorturf(target))
		return
	//try_project_paystand(user, target) // Money Rework Removal

/obj/item/card/id/attack_self_secondary(mob/user, modifiers)
	. = ..()
	if(. == SECONDARY_ATTACK_CANCEL_ATTACK_CHAIN)
		return
	//try_project_paystand(user) // Money Rework Removal
...
/obj/item/card/id/add_context(atom/source, list/context, obj/item/held_item, mob/user)
	...
	//context[SCREENTIP_CONTEXT_RMB] = "Project pay stand" // Money Rework Removal
	...
	//else if(registered_account.account_balance > 0) // Money Rework Removal
		//context[SCREENTIP_CONTEXT_ALT_LMB] = "Withdraw credits" // Money Rework Removal
...
/obj/item/card/id/click_alt(mob/living/user)
	...
	if(registered_account.account_debt)
		var/choice = tgui_alert(user, "Choose An Action", "Bank Account", list("Pay Debt")) // Money Rework Edit - ORIGINAL - var/choice = tgui_alert(user, "Choose An Action", "Bank Account", list("Withdraw", "Pay Debt"))
	...
	if(registered_account.replaceable && !registered_account.account_balance)
		...
		if(choice == "Link Account")
			...
			return CLICK_ACTION_SUCCESS
			/* Money Rework Removal - Holochips
...
/obj/item/card/id/examine_more(mob/user)
	...
	if(registered_account)
		...
		//. += span_info("Alt-Click the ID to pull money from the linked account in the form of holochips.") // Money Rework Edit
		//. += span_info("You can insert credits into the linked account by pressing holochips, cash, or coins against the ID.") // Money Rework Edit
